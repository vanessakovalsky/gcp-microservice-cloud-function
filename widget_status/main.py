...
def post_widget_status(request_context):
        # obtain the widget identifier to create 
        widget_uid = request_context.get('widget_uid')
        widget_status_code = request_context.get('widget_status_code','PROCESSING')

        # Remember to close SQL resources declared while running this function.
        # Keep any declared in global scope (e.g. mysql_conn) for later reuse.
        with __get_cursor() as cursor:
            cursor.execute("""INSERT INTO widget_db.widget_status (widget_uid, status_code)
                               VALUES(%s, %s) """,(widget_uid, widget_status_code))
...
def widget_status(request):
    try :
        ...
        if request.method == 'GET' :
            ...
            response = post_widget_status(request_context) 
